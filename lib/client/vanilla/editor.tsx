...
import { handleData2 } from '../../server/api/handle'
import { handleTheme, loadTemplate, savePageAction } from '../handle/actions'
...
const getBaseUrl = (standaloneServer: boolean) => {
  return standaloneServer ? `https://12785-liveduo-destack-6mvchdvoz52.ws-us108.gitpod.io` : ''
}
...
function Editor({ standaloneServer = false }) {
  ...
  useEffect(() => {
    loadPage().then((html?: any) => {
      return (canvasRef.current!.innerHTML = html)
    })
